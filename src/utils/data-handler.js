...
export const isObjectEmpty = (o) => o === null || Object.entries(o).length === 0
/**
...
window.Editor_dispatchDataEventToLocalStorage = (key, config) => {
  ...
  const configToStore = typeof config === 'object' && !(Object.entries(config).length === 0) ? config : fallbackConfig
  localStorage.setItem(keyToStore, JSON.stringify(configToStore))
...
window.Editor_getDataFromLocalStorage = (key) => {
  ...
  return JSON.parse(localStorage.getItem(keyToSearch))
}
